function businessCertifications(){
    ...
    IMP.certification({
        ...
        if ( rsp.success ){ // 인증에 성공할 경우
            ...
            let url = "/business/signUpPage/certifications/";
...
function resultCertifications () {
    if ( xhr.readyState == 4 && xhr.status == 200 ) {
        ...
        let placeCall = document.getElementById("placeCall"); // 휴대폰 번호
        ...
        placeCall.value = jsData.placeCall; // 변환된 Map 값 중 휴대폰 번호를 가져와 휴대폰 번호 작성란에 전달한다.
        placeCall.readOnly = true; // 전달된 휴대폰 번호를 변경하지 못하도록 막는다.

        // 35. Ajax를 사용하여 33에서 변환된 Map 값 중 휴대폰 번호를 컨트롤러로 보낸다. - 중복 가입자인지 체크
        let url = "/business/signUpPage/placeCallCheck";
        let param = "placeCall=" + jsData.placeCall;
...
function resultCheckPhone() {
    if ( xhr.readyState == 4 && xhr.status == 200 ) {
        ...
        if( data == "no" ) {
            ...
            placeCall.value = ""; // 작성된 휴대폰 번호를 초기화한다.
            return;
